...
.. _qtl_and_allele_effects:

QTL and Allele Effects
======================

For purposes of development I am using a simple exponential distribution
with a reasonable amount of QTL.

.. code-block:: py
   :caption: Choosing QTL and allele effects

   >>> qtl = random.sample(tuple(range(tuson.totNumLoci())), 50)
...
We initialize some variables to store the multi-generation data. Note that for
sampling generations we do not need to include the first and last generations.
In the past I stored the meta-population in an actual simuPOP.Simulator. Each
generation was a sub-population. However, there is no reason to do that. Now
the meta-population is stored ina a list keyed by the replicate id.
...
In the case of 5 replicates and 10 generations of selection:
...
For the time being I will collect the following types of data.

Minor Allele Frequencies
^^^^^^^^^^^^^^^^^^^^^^^^

+ :py:func:`collect_allele_frequency_data`
+ :py:func:`store_allele_frequency_data`

Heterozygote Frequencies
^^^^^^^^^^^^^^^^^^^^^^^^

+ :py:func:`collect_heterozygote_frequency_data`
+ :py:func:`store_heterozygote_frequency_data`

Minor Allele Homozygote Frequencies
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
I wrote different functions to have alternative ways of storage. Functions
prefixed by collect\_ are the *old* way where the data is collected:
in a single array. The functions prefixed by store\_ put the data into an HDF5
